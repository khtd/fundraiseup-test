...
import { DB_CUSTOMERS_COLLECTION_NAME, DB_NAME, NEW_CUSTOMERS_MAX, NEW_CUSTOMERS_MIN, GENERATION_INTERVAL_MS } from './common/constraints';
...
function getRandomAmount() {
  return Math.floor(Math.random() * (NEW_CUSTOMERS_MAX - NEW_CUSTOMERS_MIN)) + NEW_CUSTOMERS_MIN;
}

async function execute() {
  const newCustomers = createMultipleCustomers(getRandomAmount())

  const collection = client.db(DB_NAME).collection<ICustomer>(DB_CUSTOMERS_COLLECTION_NAME);
